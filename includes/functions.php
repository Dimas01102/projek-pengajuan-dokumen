...
// Helper untuk encode/decode deskripsi dengan field config
function encode_deskripsi_with_config($deskripsi_text, $field_config_array) {
    $field_json = json_encode($field_config_array);
    return $deskripsi_text . "###JSON_CONFIG###" . $field_json;
}
...
function decode_deskripsi_with_config($deskripsi_gabungan) {
    if (strpos($deskripsi_gabungan, "###JSON_CONFIG###") !== false) {
        ...
        return [
            'deskripsi' => $parts[0],
            'field_config' => json_decode($parts[1], true) ?: []
        ];
    }
    return [
